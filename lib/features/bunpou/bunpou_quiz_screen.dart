...
enum BunpouQuizMode { patternToMeaning, meaningToPattern }

/// Multiple-choice quiz over one level's grammar patterns, in one of two
/// modes: shown the pattern and asked to pick its meaning, or shown the
...
class _BunpouQuizScreenState extends ConsumerState<BunpouQuizScreen> {
  ...
  List<_QuizQuestion> _buildQuestions() {
    final random = Random();
    final pool = List<BunpouEntry>.from(widget.entries)..shuffle(random);
    final chosen = pool.take(min(_questionCount, pool.length)).toList();

    return chosen.map((entry) {
      final distractorPool = widget.entries.where((b) => b.id != entry.id).toList()..shuffle(random);
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                if (_isPatternToMeaning) ...[
                  const SizedBox(height: 4),
                  Text(
                    question.entry.patternRomaji,
          ...
          child: Row(
            children: [
              Expanded(
                child: Text(
                  text,
                  style: TextStyle(fontSize: large ? 20 : 14, color: textColor),
                ),
              ),
              if (icon != null)
                Icon(
