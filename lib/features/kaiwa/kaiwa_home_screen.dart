...
class _LevelCard extends ConsumerWidget {
  ...
  void _open(BuildContext context, AppStrings s, bool gateReached) {
    ...
    if (!gateReached) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(s.kaiwaLevelLockedReason(previousLevel.key))),
      );
      return;
    }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final thisLevel = JlptLevelX.fromKey(level.id);
    final gates = ref.watch(kaiwaLevelGateProvider).valueOrNull;
    ...
    final progress = available
        ? ref.watch(kaiwaLevelProgressProvider(thisLevel)).valueOrNull
        : null;
    final s = ref.watch(appStringsProvider);
    final total = progress?.$2 ?? level.themeCount ?? 0;
    final learned = progress?.$1 ?? 0;
    final percent = total > 0 ? ((learned / total) * 100).round() : 0;

    return ModuleLevelCard(
      badgeLabel: level.name,
      title: s.kaiwaLevelTitle(level.name),
      subtitle: s.themeCount(level.themeCount ?? 0),
