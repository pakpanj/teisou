"""Splice scripts/reference/n2_n4_web_batch.py's 179 words into the right
generate_kotoba_*.py files as new tuples. Run from teisou project root:
python scripts/reference/apply_n2n4_batch.py
"""
import json
import re
import sys
...
from n2_n4_web_batch import N2_N4_WORDS  # noqa: E402
from hiragana_romaji import to_romaji  # noqa: E402

CATEGORY_TO_SCRIPT = {
    "cuaca": "generate_kotoba_alam.py", "bencana_alam": "generate_kotoba_alam.py",
    "bunga_tanaman": "generate_kotoba_alam.py", "burung": "generate_kotoba_alam.py",
...
def build_tuples():
    ...
    out = {}
    for kanji, reading, meaning, level, cat, ex_ja, ex_ro, ex_id in N2_N4_WORDS:
        ...
        if script in TWELVE_FIELD_SCRIPTS:
            tup = (
                ...
                f'"{level}", "noun", {kanji_lit}, {romaji_lit}, {kanji_lit}, {romaji_lit}, [\n'
                f'            ({ex_ja_lit}, {ex_ro_lit}, {ex_id_lit}),\n'
                f'        ]),\n'
            )
        else:
            tup = (
                f'        ("{suffix}", {kanji_lit}, {reading_lit}, {romaji_lit}, {meaning_lit}, "{level}", [\n'
                f'            ({ex_ja_lit}, {ex_ro_lit}, {ex_id_lit}),\n'
                f'        ]),\n'
            )
